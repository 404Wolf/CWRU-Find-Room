...
async def findRooms():
    ...
    cached_headers = cache.get("auth:cookies")
    cached_cookies = cache.get("auth:headers")
    ...
    cached_headers = json.loads(cached_headers)
    cached_cookies = json.loads(cached_cookies)
